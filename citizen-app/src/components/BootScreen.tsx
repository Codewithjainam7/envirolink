...
interface BootScreenProps {
    onComplete: () => void;
    variant?: 'citizen' | 'admin' | 'worker';
}

export default function BootScreen({ onComplete, variant = 'citizen' }: BootScreenProps) {
    const [phase, setPhase] = useState<'rotating' | 'zooming' | 'done'>('rotating');

    useEffect(() => {
        // Phase 1: Icon rotates like Discord (1.5s)
        const rotateTimer = setTimeout(() => {
            setPhase('zooming');
        }, 1800);

        // Phase 2: Logo zooms in like Netflix (0.8s) then fade out
        ...
        <AnimatePresence>
            {phase !== 'done' && (
                <motion.div
                    ...
                    className={`fixed inset-0 z-[9999] bg-gradient-to-br ${config.bg} flex items-center justify-center overflow-hidden`}
                >
                    ...
                    <div className="relative flex flex-col items-center">
                        {/* Phase 1: Rotating Icon (Discord-style) */}
                        {phase === 'rotating' && (
                            <motion.div
                                className="relative"
                        ...
                        {/* Phase 2: Zooming Logo (Netflix-style) */}
                        {phase === 'zooming' && (
                            <motion.div
